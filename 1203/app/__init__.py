...
from flask import send_from_directory, render_template, request, abort, session
...
from flask_keys import get_flask_keys
...
def csrf_protect():
    if request.method == "POST":
        token = get_flask_keys()['token']
        if token != request.form.get('_csrf_token'):
            abort(403)


def generate_csrf_token():
    return get_flask_keys()['token']
